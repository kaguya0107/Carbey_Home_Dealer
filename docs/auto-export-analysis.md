...
## 合意事項（クライアント一次回答 2026-08-13）
提案に対する一次回答。残りは数週間かけて確認予定（付録の8・9＝MVP線引き/次工程は継続確認中）。いずれも当方提案と整合。
- **既存資産の流用方針**：提案の認識でOK。
- **精算の取消・再精算**：**履歴を保持する方式で確定**（上書きしない）。
- **有料「輸出車両選択」**：後から追加できる**有料アドオンとして実装**。**既存の検索機能を活用**。**将来の決済システム自動連携へ拡張できる構成**とする。
- **多言語**：MVPは**日英2か国語**でOK。
- **アクセス権限**：**必要な機能のみに制限（最小権限）**。
- **チャット**：有効化して問題なし（※外部パートナーはデータ隔離を維持）。
- **船イメージUI**：MVP採用時は**フェーズ2送り**（＝MVPは簡易ステータス表示）。← 実現可能。
- **開発想定期間**：**MVP 4〜5週間**（既存機能を大きく流用する前提の集中開発）。確定見積は残項目（船UI/⑧・⑨）確定後に提示。
